...
public class Map extends DialogFragment {
    ...
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
        ...
        return builder
                .setView(view)
                .setTitle("Search a QRCode")
                .setNegativeButton("Cancel", null)
                .setPositiveButton("Search", (dialog, which) -> {
                    Float latitude = Float.parseFloat(la.getText().toString());
                    Float longitude = Float.parseFloat(lo.getText().toString());
//                    listener.addCity(new City(cityName, provinceName));
                    new Search().show(getSupportFragmentManager(), "Search QRCode");
                })
                .create();
    }
}
